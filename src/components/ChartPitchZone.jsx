...
import { strikeTypes } from "./PitchFilters";
import css from "../styles/ChartPitchZone.module.css";
...
export const ChartPitchZone = ({ pitchEvents }) => {
  const pitchZoneRef = React.createRef();
  const [highlightedPitch, setHighlightedPitch] = React.useState(null);
  ...
  const updateHighlightedPitch = (indexOrNull) => {
    setHighlightedPitch(indexOrNull);
  };

  const normalizePosition = (xyz) => {
    return feetToInches(xyz) * getCanvasSizeRatio();
  };

  const getCanvasSizeRatio = () => {
    let canvas = pitchZoneRef.current;
    if (!canvas) {
      return 0;
    }
  ...
  const draw = (pitchEvents) => {
    ...
    requestAnimationFrame(() => {
      ...
      const ratio = getCanvasSizeRatio();
      ...
      for (let i = 0; i < pitchEvents.length; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(
          normalizePosition(p.init_pos_x) + canvasCenterX,
          canvas.height - normalizePosition(p.init_pos_z)
        );
        ...
        ctx.quadraticCurveTo(
          normalizePosition(p.init_pos_x) + canvasCenterX + curveOffset,
          canvas.height - normalizePosition(p.init_pos_z) - curveOffset,
          normalizePosition(p.plate_x) + canvasCenterX,
          canvas.height - normalizePosition(p.plate_z)
        );
  ...
  return (
    <div className={css.host}>
      <div className={css.chartContainer}>
        <canvas ref={pitchZoneRef} height="375" width="375"></canvas>
        <img
      ...
      <div className={css.PitchList}>
        <PitchList
          pitchEvents={pitchEvents}
          onSelectionChange={updateHighlightedPitch}
        />
      </div>
    </div>
  );
